<x-app-layout>
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-xl shadow-sm p-3 flex gap-3">
            <div class="flex-1 flex items-center gap-2 px-3 py-2 bg-red-50 rounded-lg">
                <div class="w-8 h-8 bg-red-100 rounded-full flex items-center justify-center">
                    <span class="text-xs font-bold text-red-600">{{ $unpaidCount }}</span>
                </div>
                <div>
                    <p class="text-xs text-red-600 font-medium">{{ __('Belum Bayar') }}</p>
                    <p class="text-sm font-bold text-red-700">RM {{ number_format($unpaidAmount, 2) }}</p>
                </div>
            </div>
            <div class="flex-1 flex items-center gap-2 px-3 py-2 bg-green-50 rounded-lg">
                <div class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
                    <span class="text-xs font-bold text-green-600">{{ $paidCount }}</span>
                </div>
                <div>
                    <p class="text-xs text-green-600 font-medium">{{ __('Sudah Bayar') }}</p>
                    <p class="text-sm font-bold text-green-700">RM {{ number_format($paidAmount, 2) }}</p>
                </div>
            </div>
        </div>
                    ...
                    @foreach($bills as $bill)
                        <div class="p-4 flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <a href="{{ route('resident.bills.print', $bill) }}" target="_blank" class="w-9 h-9 flex items-center justify-center rounded-lg text-gray-400 hover:text-primary-600 hover:bg-primary-50 transition" title="{{ __('Cetak Bil') }}">
