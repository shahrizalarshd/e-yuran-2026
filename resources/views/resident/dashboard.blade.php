<x-app-layout>
    ...
    <div class="space-y-4 lg:space-y-6" x-data="dashboardBills()">
        ...
        <!-- Yearly Progress Bar -->
        @if($yearlyProgress['total'] > 0)
        <div class="bg-white rounded-xl shadow-sm p-4">
            <div class="flex items-center justify-between mb-2">
                <h3 class="font-semibold text-gray-900 text-sm">{{ __('Progres Tahun') }} {{ $currentYear }}</h3>
                <span class="text-sm font-medium {{ $yearlyProgress['percentage'] === 100 ? 'text-green-600' : 'text-primary-600' }}">
                    {{ $yearlyProgress['paid'] }}/{{ $yearlyProgress['total'] }} {{ __('bulan') }}
                </span>
            </div>
            <div class="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                <div class="h-3 rounded-full transition-all duration-500 {{ $yearlyProgress['percentage'] === 100 ? 'bg-green-500' : 'bg-primary-500' }}" style="width: {{ $yearlyProgress['percentage'] }}%"></div>
            </div>
            <div class="flex items-center justify-between mt-2 text-xs text-gray-500">
                <span>{{ __('Sudah bayar') }}: RM {{ number_format($yearlyProgress['paidAmount'], 2) }}</span>
                <span>{{ __('Jumlah') }}: RM {{ number_format($yearlyProgress['totalAmount'], 2) }}</span>
            </div>
            @if($yearlyProgress['percentage'] === 100)
                <div class="mt-2 flex items-center gap-1.5 text-xs text-green-600 font-medium">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    {{ __('Semua bil tahun ini sudah selesai!') }}
                </div>
            @endif
        </div>
        @endif
        ...
        <div class="grid grid-cols-3 gap-2 lg:gap-4">
            <a href="{{ route('resident.payments.create', ['type' => 'current']) }}" class="bg-white rounded-xl p-3 lg:p-4 shadow-sm hover:shadow-md transition flex flex-col lg:flex-row items-center gap-2 lg:gap-4 text-center lg:text-left">
                ...
                <div>
                    <p class="font-medium text-gray-900 text-xs lg:text-sm">{{ __('messages.current_month') }}</p>
                    <p class="text-xs text-gray-500 hidden lg:block">{{ __('Bayar bulan semasa') }}</p>
                </div>
            </a>

            <a href="{{ route('resident.payments.create', ['type' => 'selected']) }}" class="bg-white rounded-xl p-3 lg:p-4 shadow-sm hover:shadow-md transition flex flex-col lg:flex-row items-center gap-2 lg:gap-4 text-center lg:text-left">
                ...
                <div>
                    <p class="font-medium text-gray-900 text-xs lg:text-sm">{{ __('messages.selected_months') }}</p>
                    <p class="text-xs text-gray-500 hidden lg:block">{{ __('Pilih bulan untuk bayar') }}</p>
                </div>
            </a>

            <a href="{{ route('resident.payments.create', ['type' => 'yearly']) }}" class="bg-white rounded-xl p-3 lg:p-4 shadow-sm hover:shadow-md transition flex flex-col lg:flex-row items-center gap-2 lg:gap-4 text-center lg:text-left">
                ...
                <div>
                    <p class="font-medium text-gray-900 text-xs lg:text-sm">{{ __('messages.yearly') }}</p>
                    <p class="text-xs text-gray-500 hidden lg:block">{{ __('Bayar setahun penuh') }}</p>
                </div>
            </a>
        </div>

        <!-- Unpaid Bills (Grouped by Year, Collapsible) -->
        @if($unpaidBills->count() > 0)
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                <div class="p-4 border-b border-gray-100 flex items-center justify-between">
                    <div>
                        <h2 class="font-semibold text-gray-900">{{ __('messages.unpaid') }}</h2>
                        <p class="text-xs text-gray-500 mt-0.5">{{ $unpaidBills->count() }} {{ __('bil') }} · RM {{ number_format($outstandingAmount, 2) }}</p>
                    </div>
                    @if($primaryMembership->can_pay)
                        <a href="{{ route('resident.payments.create') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-600 text-white text-xs font-semibold rounded-lg hover:bg-red-700 transition">
                ...
                @foreach($shownYears as $year => $yearBills)
                    ...
                    <button type="button" @click="toggle({{ $year }})" class="w-full p-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between hover:bg-gray-100 transition">
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-gray-500 transition-transform duration-200" :class="{ 'rotate-90': expanded.includes({{ $year }}) }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                            </svg>
                            <span class="font-semibold text-gray-700">{{ $year }}</span>
                            <span class="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-medium">{{ $yearBills->count() }} {{ __('bil') }}</span>
                        </div>
                        <span class="text-sm font-medium text-red-600">RM {{ number_format($yearBills->sum('outstanding_amount'), 2) }}</span>
                    </button>

                    <!-- Year Group Bills -->
                    <div x-show="expanded.includes({{ $year }})" x-transition class="divide-y divide-gray-100">
                        @foreach($yearBills->sortBy('bill_month') as $bill)
                            <div class="p-4 flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($primaryMembership->can_pay)
                                        <a href="{{ route('resident.payments.create', ['type' => 'selected']) }}" class="w-9 h-9 flex items-center justify-center rounded-lg bg-primary-50 text-primary-600 hover:bg-primary-100 transition" title="{{ __('messages.pay_now') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Charts Section (moved below bills) -->
    ...
    @push('scripts')
    <script>
        // Dashboard Alpine.js component for collapsible bill groups
        function dashboardBills() {
            const currentYear = {{ $currentYear }};
            return {
                expanded: [currentYear], // Only current year expanded by default
                toggle(year) {
                    const idx = this.expanded.indexOf(year);
                    if (idx > -1) {
                        this.expanded.splice(idx, 1);
                    } else {
                        this.expanded.push(year);
                    }
                }
            };
        }
