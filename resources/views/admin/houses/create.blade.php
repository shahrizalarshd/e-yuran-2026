<x-app-layout>
    <x-slot name="title">{{ __('messages.add_house') }}</x-slot>

    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            <div class="p-4 lg:p-6 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-900">{{ __('messages.add_house') }}</h2>
                <p class="text-sm text-gray-500 mt-1">{{ __('Maklumat rumah baru') }}</p>
            </div>

            <form action="{{ route('admin.houses.store') }}" method="POST" class="p-4 lg:p-6 space-y-6">
                @csrf
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="house_no" id="house_no" value="{{ old('house_no') }}" required class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500" placeholder="1A">
                        @error('house_no')
                    ...
                    <div>
                        ...
                        <select name="street_name" id="street_name" required class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500">
                            ...
                            <option value="Jalan Tropika 2" {{ old('street_name') === 'Jalan Tropika 2' ? 'selected' : '' }}>Jalan Tropika 2</option>
                            <option value="Jalan Tropika 3" {{ old('street_name') === 'Jalan Tropika 3' ? 'selected' : '' }}>Jalan Tropika 3</option>
                            <option value="Jalan Tropika 4" {{ old('street_name') === 'Jalan Tropika 4' ? 'selected' : '' }}>Jalan Tropika 4</option>
                            <option value="Jalan Tropika 5" {{ old('street_name') === 'Jalan Tropika 5' ? 'selected' : '' }}>Jalan Tropika 5</option>
                        </select>
                        @error('street_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500">
                        ...
                        <option value="vacant" {{ old('status') === 'vacant' ? 'selected' : '' }}>{{ __('messages.vacant') }}</option>
                    </select>
                </div>

                <div class="space-y-4">
                    <label class="flex items-center gap-3 p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 min-h-touch">
                        <input type="checkbox" name="is_registered" value="1" {{ old('is_registered') ? 'checked' : '' }} class="w-5 h-5 text-primary-600 border-gray-300 rounded focus:ring-primary-500">
                        <div>
                ...
                <div class="flex gap-3 pt-4 border-t border-gray-200">
                    <a href="{{ route('admin.houses.index') }}" class="flex-1 py-3 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-gray-200 transition text-center min-h-touch">
                        {{ __('messages.cancel') }}
                    </a>
